...
internal class ClipboardHistoryDatabase private constructor(context: Context) :
    SQLiteOpenHelper(context, "clipboard_history.db", null, 1) {
    companion object {
    ...
    fun queryGroup(db: SQLiteDatabase, pinned: Boolean): List<ClipboardHistoryStore.Entry> =
        db.query(TABLE, null, "$PINNED = ?", arrayOf(if (pinned) "1" else "0"), null, null,
            "$TS DESC, $ID DESC").use(::readEntries)

    fun queryById(db: SQLiteDatabase, id: String): ClipboardHistoryStore.Entry? =
        db.query(TABLE, null, "$ID = ?", arrayOf(id), null, null, null, "1").use {
            if (it.moveToFirst()) toEntry(it) else null
        }

    fun queryLatestHistory(db: SQLiteDatabase): ClipboardHistoryStore.Entry? =
        db.query(TABLE, null, "$PINNED = 0", null, null, null, "$TS DESC, $ID DESC", "1").use {
            if (it.moveToFirst()) toEntry(it) else null
        }

    fun queryLatestFileHistory(db: SQLiteDatabase): ClipboardHistoryStore.Entry? =
        db.query(
            TABLE,
            null,
            "$PINNED = 0 AND $TYPE != ?",
    ...
    fun setPinned(db: SQLiteDatabase, id: String, pinned: Boolean, timestamp: Long): Int = db.update(
        TABLE, ContentValues().apply { put(PINNED, if (pinned) 1 else 0); put(TS, timestamp) },
        "$ID = ?", arrayOf(id)
    )

    fun setFileState(db: SQLiteDatabase, id: String, localPath: String?, status: DownloadStatus): Int =
        db.update(TABLE, ContentValues().apply {
            if (localPath != null) put(LOCAL_PATH, localPath)
            put(DOWNLOAD, status.name)
        }, "$ID = ?", arrayOf(id))

    fun deleteHistoryBefore(db: SQLiteDatabase, cutoff: Long): Int =
        db.delete(TABLE, "$PINNED = 0 AND $TS < ?", arrayOf(cutoff.toString()))
    fun deleteHistoryById(db: SQLiteDatabase, id: String): Int =
        db.delete(TABLE, "$ID = ? AND $PINNED = 0", arrayOf(id))
    ...
    fun deleteNonTextHistory(db: SQLiteDatabase): Int =
        db.delete(TABLE, "$PINNED = 0 AND $TYPE != ?", arrayOf(EntryType.TEXT.name))
    fun deleteAll(db: SQLiteDatabase) { db.delete(TABLE, null, null) }
    ...
    fun pruneGroup(db: SQLiteDatabase, pinned: Boolean, maxEntries: Int) {
        ...
        """.trimIndent(), arrayOf(if (pinned) 1 else 0, maxEntries))
    }
    ...
    private fun values(entry: ClipboardHistoryStore.Entry) = ContentValues().apply {
        put(ID, entry.id); put(TEXT, entry.text); put(TS, entry.ts); put(PINNED, if (entry.pinned) 1 else 0)
        put(TYPE, entry.type.name); putNullable(FILE_NAME, entry.fileName); putNullable(FILE_SIZE, entry.fileSize)
        putNullable(MIME, entry.mimeType); putNullable(LOCAL_PATH, entry.localFilePath)
        put(DOWNLOAD, entry.downloadStatus.name); putNullable(SERVER_NAME, entry.serverFileName)
    ...
    private fun Cursor.string(column: String): String? { val i = getColumnIndexOrThrow(column); return if (isNull(i)) null else getString(i) }
    private fun Cursor.long(column: String): Long = getLong(getColumnIndexOrThrow(column))
    private fun Cursor.nullableLong(column: String): Long? { val i = getColumnIndexOrThrow(column); return if (isNull(i)) null else getLong(i) }
