...
- **硅基流动新模型**：免费识别新增星辰 ASR 系列，BYOK 新增 Qwen3-ASR 1.7B
- **DashScope 恢复 Qwen3**：重新提供 Qwen3-ASR-Flash 非流式与 Qwen3-ASR-Flash-Realtime 流式模型
- **[Pro] 热词注入润色 Prompt**：可将已启用的热词及其别名追加到 AI 润色提示词；可选仅注入读音相近的热词

### 改进优化

- **识别与润色预热**：开始录音时预热在线识别与 AI 润色连接；录音结束释放改为异步，松手后更快出字、润色更少等待
- **录音更稳**：预热时输入源探测更快速；预加载离线降噪

### 问题修复

- **数字键盘高度**：修复数字键盘与主键盘高度不对齐问题
- **剪贴板附件同步**：减少对监听文件夹的重复处理
- **识别历史分区崩溃**：跨零点后同一条记录不再同时落入「近 2 小时」与「今天」，避免历史页崩溃
- **输入法按键显示问题**：修复某些情况下出现输入法按键显示异常的问题
- **键盘识别耗时**：修正键盘识别显示的识别阶段耗时的数据来源
